...
When you apply an `AttributeBuff`, the time left for the buff will not be altered in any way until you will call `AttributeContainer.tick()`.

`AttributeContainer.tick()` will then decrement the `duration` of all `AttributeBuff`s that have a `duration` greater than `0` by the specified `tick` value.
